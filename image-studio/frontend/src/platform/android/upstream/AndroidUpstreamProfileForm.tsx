  ...
  return (
    <section className="android-upstream-form" aria-label="编辑上游配置">
      ...
      {draft.apiMode === "images" ? (
        <>
          <AndroidField
            label="Images API 异步轮询"
            hint="适合返回 task_id/id 后再 GET /v1/images/{task_id} 的中转。"
          >
            <button
              type="button"
              className={`android-upstream-compat-toggle ${draft.imagesAsyncPolling ? "active" : ""}`}
              onClick={() => onPatchDraft({ imagesAsyncPolling: !(draft.imagesAsyncPolling === true) })}
            >
              <span>
                <strong>提交 async=true 后自动轮询任务结果</strong>
                <small>你的 Tokens Trans 上游文档写明支持同步与异步，建议开启。</small>
              </span>
              <em>{draft.imagesAsyncPolling ? "已开启" : "已关闭"}</em>
            </button>
          </AndroidField>
      ...
      <AndroidField
        label="允许 HTTP 上游"
        hint="默认只允许 HTTPS，或 localhost / 127.0.0.1 的 HTTP。"
      >
        <button
          type="button"
          className={`android-upstream-compat-toggle ${draft.allowInsecureHTTP ? "active" : ""}`}
          onClick={() => onPatchDraft({ allowInsecureHTTP: !(draft.allowInsecureHTTP === true) })}
        >
          <span>
            <strong>允许非本机 http:// BASE_URL</strong>
            <small>仅信任网络时开启；API Key 与提示词会以明文 HTTP 发送。</small>
          </span>
          <em>{draft.allowInsecureHTTP ? "已开启" : "已关闭"}</em>
        </button>
      </AndroidField>
