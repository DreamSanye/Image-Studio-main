...
export async function buildImagesRequestBody(
  ...
  const useAsyncPolling = shouldUseImagesAsyncPolling(request.payload);
  ...
  if (mode === "edit") {
    ...
    if (useNewAPICompat || useAsyncPolling || supportsImagesResponseFormat(imageModel, mode)) {
    ...
    return { url: withImagesAsyncQuery(`${baseURL}/v1/images/edits`, useAsyncPolling), body: form };
  }
  ...
  return {
    url: withImagesAsyncQuery(`${baseURL}/v1/images/generations`, useAsyncPolling),
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  };
}

function withImagesAsyncQuery(raw: string, enabled: boolean): string {
