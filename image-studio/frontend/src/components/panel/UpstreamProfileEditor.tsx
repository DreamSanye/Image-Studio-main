  ...
  return (
    <div className={`upstream-profile-editor flex min-w-0 flex-col ${isAndroidPhone ? "gap-3" : "gap-3.5"}`}>
      ...
      {draft.apiMode === "images" ? (
        <>
          <Field label="Images API 异步轮询">
            <button
              type="button"
              role="switch"
              aria-checked={draft.imagesAsyncPolling === true}
              onClick={() => onPatchDraft({ imagesAsyncPolling: !(draft.imagesAsyncPolling === true) })}
            ...
            >
              <span className="min-w-0">
                <span className="block min-w-0 break-words font-medium">提交 async=true 后自动轮询任务结果</span>
                <span className="mt-1 block min-w-0 break-words text-[11px] leading-relaxed text-zinc-500 dark:text-zinc-400">
                  适合 Tokens Trans 这类返回 <code className="font-mono-token">task_id</code> / <code className="font-mono-token">id</code> 的中转；完成后读取 <code className="font-mono-token">/v1/images/&#123;task_id&#125;</code>。
                </span>
              </span>
              <SwitchPill enabled={draft.imagesAsyncPolling === true} usesFluentUI={usesFluentUI} />
            </button>
            <Hint>默认关闭。你的 135.125.222.72:8080 上游文档写明支持同步与异步，建议这里开启。</Hint>
          </Field>
          ...
          <Field label="Images API 中转兼容">
            ...
            >
              ...
              <SwitchPill enabled={draft.imagesNewAPICompat === true} usesFluentUI={usesFluentUI} />
            </button>
            <Hint>
      ...
      <Field label="允许 HTTP 上游">
        <button
          type="button"
          role="switch"
          aria-checked={draft.allowInsecureHTTP === true}
          onClick={() => onPatchDraft({ allowInsecureHTTP: !(draft.allowInsecureHTTP === true) })}
          className={`flex w-full items-center justify-between gap-3 border border-black/[0.08] bg-[var(--surface)] px-3 py-2.5 text-left text-sm text-zinc-900 transition-colors hover:border-[color:var(--accent)]/35 dark:border-white/[0.08] dark:text-zinc-100 ${usesFluentUI ? "rounded-[10px]" : "rounded-[14px]"}`}
        >
          <span className="min-w-0">
            <span className="block min-w-0 break-words font-medium">允许非本机 http:// BASE_URL</span>
            <span className="mt-1 block min-w-0 break-words text-[11px] leading-relaxed text-zinc-500 dark:text-zinc-400">
              仅在你信任该网络和中转站时开启；API Key 与提示词会以明文 HTTP 发送。
            </span>
          </span>
          <SwitchPill enabled={draft.allowInsecureHTTP === true} usesFluentUI={usesFluentUI} />
        </button>
        <Hint>默认只允许 HTTPS，或 localhost / 127.0.0.1 的 HTTP。</Hint>
      </Field>
...
function SwitchPill({ enabled, usesFluentUI }: { enabled: boolean; usesFluentUI: boolean }) {
  return (
    <span
