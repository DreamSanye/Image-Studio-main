...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  submit: async () => {
    ...
    const basePayload: GenerateOptionsLike = {
      ...
      imagesAsyncPolling: s.imagesAsyncPolling,
      ...
      fallbackProfile: fallbackProfile && fallbackProfileKey.trim() && fallbackProfile.baseURL.trim()
        ? {
            ...
            imagesAsyncPolling: fallbackProfile.imagesAsyncPolling === true,
            allowInsecureHTTP: fallbackProfile.allowInsecureHTTP === true,
          }
        : undefined,
  ...
  bootstrap: async () => {
    ...
    const imagesAsyncPolling = activeProfile?.imagesAsyncPolling === true;
    const allowInsecureHTTP = activeProfile?.allowInsecureHTTP === true;
    ...
    set({
      ...
      apiMode, responsesTransport, requestPolicy, imagesNewAPICompat, imagesAsyncPolling, allowInsecureHTTP, baseURL, textModelID, imageModelID, reasoningEffort, kernelRuntimeMode, noPromptRevision,
  ...
  optimizePrompt: async () => {
    ...
    let optimizeAllowInsecureHTTP = s.allowInsecureHTTP;
    if (s.apiMode !== "responses") {
      ...
      if (responsesProfile) {
        ...
        optimizeAllowInsecureHTTP = responsesProfile.allowInsecureHTTP === true;
    ...
    try {
      const optimized = await wailsOptimizePrompt({
        ...
        allowInsecureHTTP: optimizeAllowInsecureHTTP,
        proxyMode: s.proxyMode,
        proxyURL: s.proxyURL,
