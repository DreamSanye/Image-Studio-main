...
func TestRequestImagesAPIAsyncPolling(t *testing.T) {
	finalB64 := base64.StdEncoding.EncodeToString([]byte("final"))
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/images/generations":
			if r.URL.Query().Get("async") != "true" {
				t.Fatalf("missing async query: %s", r.URL.RawQuery)
			}
	...
	if requestBody["async"] != true {
		t.Fatalf("request body missing async=true: %#v", requestBody)
	}
	if _, ok := requestBody["stream"]; ok {
		t.Fatalf("async request should omit stream: %#v", requestBody)
	}
	if got := requestBody["response_format"]; got != "b64_json" {
		t.Fatalf("response_format = %#v, want b64_json", got)
	}
	if pollHits != 4 {
		t.Fatalf("pollHits = %d, want 4", pollHits)
	}
	if res.ImageB64 != finalB64 || res.RevisedPrompt != "done" {
		t.Fatalf("unexpected result: %+v", res)
	}
	if !strings.Contains(raw.String(), "--- images-task-task_abc-poll-4 ---") {
