...
type BaseURLPolicy struct {
	// AllowInsecureHTTP permits http:// upstreams for non-loopback hosts.
	// Keep this off unless the user explicitly trusts the network path.
...
func ValidateBaseURL(raw string) (string, error) {
	return ValidateBaseURLWithPolicy(raw, BaseURLPolicy{})
}

func ValidateBaseURLWithPolicy(raw string, policy BaseURLPolicy) (string, error) {
	...
	case "https":
		return cleaned, nil
	case "http":
		if isLoopbackHost(u.Hostname()) || policy.AllowInsecureHTTP {
			return cleaned, nil
		}
