...
func requestResponsesWithWebSocketReplay(
	...
	result, err := requestResponsesOverWebSocket(ctx, baseURL, opts.APIKey, opts.Proxy, payload, rawSink, onPartial, snapshot, opts.AllowInsecureHTTP)
...
func ProbeResponsesWebSocket(ctx context.Context, opts ProbeResponsesWebSocketOptions) error {
	...
	return probeResponsesWebSocketOnce(ctx, opts.BaseURL, opts.APIKey, opts.Proxy, payload, opts.AllowInsecureHTTP)
}
...
func requestResponsesOverWebSocket(
	...
	for reconnect := 0; reconnect <= websocketQuickReconnectAttempts; reconnect++ {
		...
		result, err := requestResponsesOverWebSocketOnce(ctx, baseURL, apiKey, proxy, payload, rawSink, onPartial, snapshot, allowInsecureHTTP)
		if err == nil {
			return result, nil
		}
		if rawSink != nil {
...
func requestResponsesOverWebSocketOnce(
	...
	wsURL, err := responsesWebSocketURL(baseURL, allowInsecureHTTP)
	if err != nil {
		return ImageResult{}, err
	}
...
func responsesWebSocketURL(baseURL string, allowInsecureHTTP bool) (string, error) {
	normalized, err := ValidateBaseURLWithPolicy(baseURL, BaseURLPolicy{AllowInsecureHTTP: allowInsecureHTTP})
	if err != nil {
		return "", err
	}
