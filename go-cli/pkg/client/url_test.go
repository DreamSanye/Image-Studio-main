...
func TestValidateBaseURLWithPolicyAllowsRemoteHTTP(t *testing.T) {
	t.Parallel()

	got, err := ValidateBaseURLWithPolicy("http://135.125.222.72:8080/v1/", BaseURLPolicy{AllowInsecureHTTP: true})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got != "http://135.125.222.72:8080" {
		t.Fatalf("got %q", got)
	}
}
